...
float characDerivation = 0.0, characNewValue = 0.0, characOldValue = 0.0;
int characActivation=-1,characStatus=-1;
unsigned long characHigh,characLow;

float characThreshold=0.5;
...
if ((micros()-t0)>=dt) {   /*Everything inside this if statment happens at the same time as the acquisition */
  ...
  Serial.print(",");
  Serial.println(characActivation);
}
...
void FeatureExtraction  (){
  characOldValue=characNewValue;
  characNewValue=values.smooth;
  characDerivation=characNewValue-characOldValue;
  if((characDerivation>characThreshold)&&(characActivation==-1)){
    if(characStatus=-1){
      characHigh=values.times;  /*Time where the curve turn "up"*/
    }
    characStatus=1;
  ...
  else if((characDerivation<(-(characThreshold)))&&(characActivation==1)){
    if(characStatus=1){
      characLow=values.times;  /*Time where the curve turn "up"*/
    }
    characStatus=-1;
    
    characActivation=-1;
    characLow=values.times; /*Time where the curve turn "down"*/
  }
